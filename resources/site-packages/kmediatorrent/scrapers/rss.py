...
def parse(data, content_type=None):
    ...
    for node in root.getiterator("item"):
        item = {
            ...
            "seeds": _text(node, ".//{%(torrent)s}seeds" % NSMAP) or _text(node, "torrent:seeds") or _text(node, "numSeeders") or _text(node, "seeders"),
            "peers": _text(node, ".//{%(torrent)s}peers" % NSMAP) or _text(node, "torrent:peers") or _text(node, "numLeechers") or _text(node, "leechers"),
            "content_length": _text(node, ".//{%(torrent)s}contentLength" % NSMAP) or _text(node, "torrent:contentLength") or _attr(node, ".//enclosure", "length"),
    ...
    for item, tmdb_data in izip_longest(items, tmdb_list):
        if tmdb_data:
            ...
            title = "";
            if (item["title"].find("1080p") > 0):
                title = "1080p"

            if (item["title"].find("720p") > 0):
                title = "720p"

            list_item = {
                "label": "[" + str(round(int(item["content_length"])/ 10000000)/100) + "GB] " + item["seeds"] + "/" + item["peers"] + " " + title ,
